...
data_barrios = gpd.read_file(
    r'D:\casas\Documents\Proyectos\AOS\FACHADAS\analysis_facade-street\01-geodata\geodata-inicial'
    r'\BARRIOS.geojson')
data_barrios['geometry'] = data_barrios['geometry'].centroid

data_barrios.to_file(
    r"C:\Users\casas\Desktop\extract-windows\01-geodata\geodata-generada\data_barrios.geojson",
    driver="GeoJSON")
...
# Edad edificación
building_year = pd.read_excel(
    r'D:\casas\Documents\Proyectos\AOS\FACHADAS\analysis_facade-street\02-datasets\datasets-inicial\BB_EE_Barrios.xlsx')
building_year.drop_duplicates(subset="gml_id", keep=False, inplace=True)

propiedades_entidad = pd.merge(csv, building_year, on="gml_id", how="left")
