...
@hops.component(
    "/execute_query", name="execute_query",
    description="insert query for work with postgres databases",
    inputs=[
...
def send_general_building_data(city, local_id, facade_area, roof_area, batch_id):
    ...
    query_insert = f"""
        INSERT INTO city_simulations.general_building_data (city, local_id, facade_area, 
        roof_area, batch_id)
        VALUES (%s, %s, %s, %s, %s)
...
def send_facade_simulation_data(local_id, avg_roof_radiation, avg_facade_radiation, floor_number,
    ...
    # Facade simulation results
    vars_facade_simulation_results = zip(local_id, avg_roof_radiation, avg_facade_radiation,
                                         floor_number,
