...
- **Data Collection Stage**: The process begins with the collection of GIS data from Catastro, which
  includes building and block information for Logroño.
- **Building Data**: The building data is imported and classified based on the building part ID and
  height.
- **Block Data**: The block data is processed to create block information based on the block ID.
- **Segmentation**: The information is segmented using a grid, with each grid cell classified based
  on the intersection with buildings and blocks.
...
> [!NOTE]
> The data creation process involves importing and classifying GIS data from Catastro,
> segmenting the data using a grid, and creating a 3D model of Logroño. The Ladybug plugin is
> used in the simulation process to estimate solar exposure, shadows, and views in the city.
...
> [! IMPORTANT]
> The PostgreSQL database plays a crucial role in storing and managing the data generated by the
> GIS processing and simulation workflows. It provides a structured and efficient way to store
> the data and enables complex queries and analysis to be performed on the data.
...
- **Deployment**: The project can be deployed on a cloud platform like Azure for scalability and
  accessibility.
- **Enhancements**: Future enhancements could include additional simulation features, data
  visualization tools, and user interface improvements.
- **Integration**: The platform could be integrated with other urban planning tools and databases to
  enhance its functionality and data sources.
- **Collaboration**: Collaborating with urban stakeholders and researchers could help refine the
  platform's features and analysis capabilities.
