...
@hops.component(
    ...
    inputs=[
        ...
        hs.HopsInteger("avg_facade_radiation", "avg_facade_radiation",
                       access=hs.HopsParamAccess.LIST),
        hs.HopsInteger("floor_number", "floor_number", access=hs.HopsParamAccess.LIST),
        hs.HopsInteger("facade_area_per_floor", "facade_area_per_floor",
                       access=hs.HopsParamAccess.LIST),
        hs.HopsInteger("main_normal_vector_facade", "main_normal_vector_facade",
                       access=hs.HopsParamAccess.LIST),
        # Batch processing
...
def send_facade_simulation_data(local_id, avg_roof_radiation, avg_facade_radiation, floor_number,
                                facade_area_per_floor,
...
def send_general_building_data(local_id, roof_type, roof_grade, main_normal_roof, discarded_vectors,
                               batch_id):
    df = pd.DataFrame({
        'local_id': extract_values(local_id),
...
@hops.component(
    ...
    inputs=[
        hs.HopsNumber("source_noise_level_db", "Source noise level in dB",
                      access=hs.HopsParamAccess.LIST),
...
def calculate_noise_at_distance(source_noise_level_db, traffic_intensity, distance):
    ...
    df['adjusted_base_noise_level_db'] = df.apply(
        lambda row: row['base_noise_level_db'] + row[
            'traffic_intensity'] * DEFAULT_ADJUSTMENT_FACTOR_PER_VEHICLE,
        axis=1)
